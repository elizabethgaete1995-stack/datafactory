...
locals {
  ...
  ############################################
  # Feature flags (100% explícitos)
  ############################################
  key_cmk     = var.key_exist || var.key_custom_enabled
  lwk_enabled = var.analytics_diagnostic_monitor_enabled

  ############################################
  # Tags internos del módulo
  ############################################
  private_tags = {
  ...
  tags_inherited = merge(
    data.azurerm_resource_group.rsg_principal.tags,
    local.standard_tags,
    local.private_tags,
    var.custom_tags
  )
  ...
  parameters_adanalytics = {
    ...
    solutionTypes     = { value = ["AzureDataFactoryAnalytics"] } # evita replan/reapply
    tagsByResource    = { value = var.inherit ? local.tags_inherited : local.tags }
  }
}
...
resource "azurerm_key_vault_key" "key_generate" {
  ...
  tags = var.inherit ? local.tags_inherited : local.tags
...
# Create and configure a User Managed Identity
...
resource "azurerm_data_factory" "adf" {
  ...
  identity {
    ...
    identity_ids = local.key_cmk ? distinct(compact(concat(var.identity_list, [azurerm_user_assigned_identity.uai[0].id]))) : null
  }
